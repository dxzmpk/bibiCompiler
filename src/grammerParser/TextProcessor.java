...
public class TextProcessor {
	...
	public TextProcessor(ArrayList<String> input_cache, DefaultTableModel tbmodel_lex_result){
	...
	public void Parsing(){
		...
		while (deduce_str.size()>0 && input_cache.size()>0 ) {
			...
			// 否则的话报错
				...
				System.out.println(process+ " ERROR!  无法识别的字符"+input_cache.get(0));
				input_cache.remove(0);
			}
		}
	...
	public void getPredictMap(){
		...
		try {
			...
			predictmap = new HashMap<String, String>();
			// 采用随机读取方式
			File file = new File("src/grammerParser/file/predictldy.txt");
